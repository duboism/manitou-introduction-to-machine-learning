      ...
      \begin{itemize}
        \item $
          ...
          = \partialDeriv{}{s_{k}^{4}} \left\lbrace
            \apply{\softMaxFunc}{s_{k}^{4}} \right\rbrace
          = \partialDeriv{}{s_{k}^4} \left\lbrace
...
\begin{frame}
  ...
  \begin{textblock}{90}(5, 52)
    \begin{block}{At the previous layer}
      As $\lossFunc$ depends on $\weight_{k, p}^{2}$ through $s_{k}^{3}$ which
      influences all activations at layer 4:
      \[
      ...
      \]
      By comparison with the first step, we have:
      $
      \delta_k^3 =
