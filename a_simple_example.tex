...
\begin{frame}
  ...
  \begin{block}{How to measure performance?}<2->
    %Bad prediction: $\predLabel_i =
    %\apply{\decisionFunc_\params}{\example_i}$ is far from $\knownLabel_i$
    \begin{itemize}
    \item<3-> Define a loss function $\lossFunc: \exampleSet \times \labelSet
      \rightarrow \setR^+$ such that $\apply{\lossFunc}{
            \apply{\decisionFunc_\params}{\example_i},
            \knownLabel_i
          }$ is high if $\apply{\decisionFunc_\params}{\example_i}$ is far from
          $\knownLabel_i$
     \item<4-> Minimize $\lossFunc$ (as a function of $\params$)
     \item<5-> The correct loss function depends on the problem
    \end{itemize}
  \end{block}

  \begin{block}{In our example}<6->
    \begin{itemize}
    \item $\predLabel_i$ is a vector $\vectorext{\prob{0}, \prob{1}, \cdots,
        \prob{8}, \prob{9}}^T$
    \item<2-> $u_i$ is a vector $\vectorext{0, 0, \cdots, 1, 0}^T$ (one-hot
      encoding)
    \item<3-> Cross-entropy loss:
      \[
        \apply{\lossFunc}{\apply{\decisionFunc_\params}{\example_i}, \knownLabel_i}= - \sum_{c=1}^{10} \vectorElem{u_i}{c} \apply{\log}{\vectorElem{\predLabel_i}{c}}
     \]
    \end{itemize}
  \end{block}
\end{frame}
