...
\begin{frame}
  ...
  \begin{textblock}{100}(5,20)
    \begin{tikzpicture}[>=stealth]
      ...
      \node[text width=3cm,anchor=north west] at (\xHiddenII-1, -2.2) {{\footnotesize
      ...
      \node[text width=3cm,anchor=north west] at (\xOutput-1, -2.2) {{\footnotesize
          $s^{4}_{k} = \sum_{p=1}^{N_{3}} {\color{red} \weight^{4}_{k,p}} \output^{3}_{p}$
      }};
      \node[text width=5cm,anchor=north west] at (\xOutput-1, -3.0) {{\footnotesize
...
\begin{frame}
  ...
  \begin{textblock}{45}(5,15)
    \begin{itemize}
    \item \acsu{MLP} is a parametrized function: $\decisionFunc_{\params} :
  ...
  \begin{textblock}{50}(50,30)
    ...
    \begin{tikzpicture}[domain=0:4]
      ...
      \draw[->,color=red,thick] (\paramsInit, \lossInit) -- (\paramsInit + 0.5, \lossInit + 0.5*\grad) {};
      }
    \end{tikzpicture}
    \end{center}
  \end{textblock}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    %$
    %\nabla_\theta \frac{1}{m} \sum_{i=1}^m \ell(h_{\theta}( \mathbf{x}_i, u_i)) = 
    %\frac{1}{m} \sum_{i=1}^m \left( \frac{\partial \ell(h_{\theta}( \mathbf{x}_i, u_i)) }{\partial w_{1,1}^1},\ldots,\frac{\partial \ell(h_{\theta}( \mathbf{x}_i, u_i)) }{\partial w_{p,k}^l},\ldots,\frac{\partial \ell(h_{\theta}( \mathbf{x}_i, u_i)) }{\partial w_{N_{L-1},N_{L}}^{N_{L-1}}} \right)^T
    %$
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    %\item $\frac{\partial \ell(h_{\theta}( \mathbf{x}_i, u_i))}{\partial o_k^{L}}$: last layer, $\ell$ must be differentiable w.r.t $o_k^{L}$
